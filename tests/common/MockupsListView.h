...
namespace wpl
{
	namespace tests
	{
		namespace mocks
		{
			...
			typedef std::shared_ptr<headers_model> columns_model_ptr;
			...
			public:
				template<size_t n>
				static std::shared_ptr<headers_model> create(const column (&columns)[n], index_type sort_column,
					bool ascending)
				{	return std::shared_ptr<headers_model>(new headers_model(columns, sort_column, ascending));	}

				static std::shared_ptr<headers_model> create(const std::string &caption, short int width = 0);
				static std::shared_ptr<headers_model> create();
			...
			private:
				...
				template<size_t n>
				headers_model(const column (&columns_)[n], index_type sort_column_, bool ascending_)
			...
			inline bool operator ==(const headers_model::column &lhs, const headers_model::column &rhs)
